...
{
    return 6.0f - log2(tessFactor(dist) * 10.0f);
}
void main() {
   ...
   Tangent = normal * tangentU;
   Bitangent = normal * tangentV;

   vec3 worldPos = (model * vec4(pos,1.0f)).xyz;
   vec3 viewVecNoDisplacement = camPos - worldPos;
   float camDist = length(viewVecNoDisplacement);
   vec3 displacement = textureLod(heightMap, TexCoords, mipLevel(camDist)).x * 0.25f * Normal;
   FragPos = worldPos + (model * vec4(displacement,1.0f)).xyz;
